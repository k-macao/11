...
| --- | --- |
| Country risk and resilience | Country Instability Index (CII v8) scores with signed 24-hour movement for 31 Tier-1 countries. The Country Resilience Index ranks a 196-country public universe across 6 domains and 21 active dimensions on a 6-hour schedule. |
...
| Language coverage | The interface supports 26 languages. Arabic and Persian use right-to-left layouts. This documentation site is currently available in English and Simplified Chinese. |
...
World Monitor publishes a public, searchable [Data Source Catalog](https://worldmonitor.app/sources). Its manifest-derived inventory currently lists 530 active providers, 532 upstream hosts, 292 structured endpoints, and 268 news and OSINT feeds. The separate application inventory records 632 configured feed definitions, and the Telegram catalog enables 56 channels. These figures describe different inventory layers and should not be added together.
...
| [REST API](/api-reference) | Applications, data pipelines, and higher-volume batch reads | 36 generated service contracts and more than 200 versioned HTTP paths described by OpenAPI 3.1. Standard server-to-server access uses an API key in `X-WorldMonitor-Key`. See the reference for current routing status, then review the [quickstart](/usage-quickstart), [authentication](/usage-auth), [versioning policy](/api-versioning), and [REST rate limits](/usage-rate-limits). |
| [MCP server](/mcp-overview) | Claude, Cursor, and custom AI agents | Streamable HTTP with 60 tools, 6 workflow prompts, 10 interactive apps, and optional JMESPath response filtering. Catalog discovery is public; data calls require OAuth 2.1 or a World Monitor API key. MCP entitlements and metering are documented separately from REST API-plan allowances. |
...
| [Agent discovery](/agent-discovery) | Automated onboarding and agent-native workflows | Standard `.well-known` metadata, `llms.txt`, OpenAPI assets, an MCP server card, and a catalog of 25 installable agent skills. Discovery metadata is public; data-bearing operations still require the applicable credentials. |
...
500+ curated news feeds across geopolitics, defense, energy, think tanks, and regional news (Middle East, Africa, Latin America, Asia-Pacific), backed by 532+ observed upstream hosts.
